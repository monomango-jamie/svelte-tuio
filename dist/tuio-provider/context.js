import { getContext, setContext } from 'svelte';
/**
 * The context key used to store and retrieve the TUIOHandler instance
 */
export const TUIO_HANDLER_CONTEXT_KEY = Symbol('monomango:TUIOHandler');
/**
 * Sets the TUIOHandler instance in Svelte's context
 *
 * @param {TUIOHandler} handler - The TUIOHandler instance to store in context
 * @returns {void}
 */
export function setTUIOHandler(handler) {
    setContext(TUIO_HANDLER_CONTEXT_KEY, handler);
}
/**
 * Retrieves the TUIOHandler instance from Svelte's context
 *
 * @returns {TUIOHandler} The TUIOHandler instance
 * @throws {Error} If no TUIOHandler is found in context
 */
export function getTUIOHandlerContext() {
    const tuioHandler = getContext(TUIO_HANDLER_CONTEXT_KEY);
    if (!tuioHandler) {
        throw new Error('TUIOHandler not found. Did you forget to setTUIOHandler?');
    }
    return tuioHandler;
}
