...
<div class="max-w-full overflow-x-auto p-4 font-mono">
	...
	<section class="mb-8 rounded-lg bg-gray-100 p-4">
			...
			<div class="overflow-hidden rounded shadow-sm">
				<table class="w-full border-collapse bg-white">
					<thead class="bg-gray-700 text-white">
						<tr>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">
								Class ID
							</th>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">ID</th>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">Profile</th>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">
								Position (u, v)
							</th>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">
		...
		{:else}
			<p class="my-4 italic text-gray-600">No tangibles detected</p>
		{/if}
	</section>
	...
	<section class="mb-8 rounded-lg bg-gray-100 p-4">
		...
		{#if tuioHandler.tangiblesManager.tangibleClassIds.length > 0}
			<div class="overflow-hidden rounded shadow-sm">
				<table class="w-full border-collapse bg-white">
					<thead class="bg-gray-700 text-white">
						<tr>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">Index</th>
							<th class="p-3 text-left text-xs font-semibold uppercase tracking-wider">
								Class ID
							</th>
						</tr>
		...
		{:else}
			<p class="my-4 italic text-gray-600">No active class IDs</p>
		{/if}
	</section>
	...
	<section class="mb-8 rounded-lg bg-gray-100 p-4">
		...
		{:else}
			<p class="my-4 italic text-gray-600">No touch zones registered</p>
		{/if}
	</section>
</div>